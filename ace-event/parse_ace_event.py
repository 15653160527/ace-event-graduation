...
class Entity(TokSpan):
    ...
    def to_json(self):
        return [*self.adjusted_span_doc, self.mention_type, self.adjusted_text_string]
...
class Relation:
    ...
    def to_json(self):
        return [*self.arg1.adjusted_span_doc, *self.arg2.adjusted_span_doc, self.arg1.adjusted_text_string, self.arg2.adjusted_text_string, self.relation_type]
...
class Event:
    ...
    def to_json(self):
        ...
        trigger = [[trigger_span[0], self.trigger.trigger_type, self.trigger.adjusted_text_string]]
        args = []
        for arg in self.arguments:
            # Collapse time argument roles following Bishan.
            arg_role = "Time" if "Time" in arg.argument_role else arg.argument_role
            args.append([*arg.adjusted_span_doc, arg_role, arg.adjusted_text_string])
...
class Entry:
    ...
    def to_json(self):
        ...
        offsets = [tok.idx for tok in self.final_toks]
        sentence_start_char = self.final_toks[0].idx
        sent_str = self.sent_str
        return dict(sentences=sentences, sent_str=sent_str, ner=ner, relations=relations, events=events,
                    sentence_start=self.adjusted_start, sentence_start_char=sentence_start_char, offsets=offsets, ner_flavor=ner_flavors)
...
class Doc:
    ...
    def to_json(self):
        ...
        res = {}
        for field in ["sentences", "sent_str", "ner", "relations", "events", "sentence_start", "sentence_start_char", "offsets"]:
...
class Document:
    ...
    def _get_entry(self, sent):
        ...
        return Entry(sent=sent, sent_str=sent_str, entities=ner, relations=rel, events=events)
...
def one_fold(fold, output_dir, heads_only=True, real_entities_only=True, include_pronouns=False):
    doc_path = "./raw-data"
    split_path = "./event-split"

    doc_keys = []
    with open(path.join(split_path, fold)) as f:
        for line in f:
...
def main():
    ...
    output_dir = f"./processed-data/{args.output_name}/json"
